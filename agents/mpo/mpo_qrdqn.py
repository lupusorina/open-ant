...
try:
    from .nn_qrdqn import (AcmeActor, AcmeCritic, DiscreteValuedDistribution,  # imported as package
                     ScalarAcmeCritic, QuantileCritic, categorical, td_learning,
                     quantile_huber_loss)
except ImportError:
    from nn_qrdqn import (AcmeActor, AcmeCritic, DiscreteValuedDistribution,   # run standalone
                    ScalarAcmeCritic, QuantileCritic, categorical, td_learning,
                    quantile_huber_loss)
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--critic_snapshot_steps",
        type=int,
        default=20,
        help="Number of on-policy steps to record after each checkpoint",
    )

    parser.add_argument(
        "--record_critic_snapshots",
...
def main():
    ...
    try:
        for step in tqdm(range(agent.global_step, args.total_timesteps)):
            ...
            if (
                args.record_critic_snapshots
                and not args.eval
                and args.critic_type == "quantile"
            ):
                steps_to_checkpoint = (
                    args.save_every_n_steps
                    - (agent.global_step % args.save_every_n_steps)
                )

                if steps_to_checkpoint <= args.critic_snapshot_steps:
                    with torch.no_grad():

                        # Follow env 0 so this is one coherent trajectory.
                        state = agent.obs[0:1]
                        action = selected_actions[0:1]

                        # Quantile locations predicted by the (single) critic.
                        quantiles = agent.critics[0](state, action).squeeze(0)
            ...
            if (not args.eval and current_step % args.save_every_n_steps == 0):
                ...
                if args.record_critic_snapshots and critic_trace:
                    ...
                    tau = (
                        torch.arange(args.num_quantiles, dtype=torch.float32) + 0.5
                    ) / args.num_quantiles

                    np.savez_compressed(
                        os.path.join(
                            snapshot_dir,
                            f"critic_trajectory_{current_step}.npz",
                        ),
                        steps=np.array([x["step"] for x in critic_trace]),
                        states=np.stack([x["state"] for x in critic_trace]),
                        actions=np.stack([x["action"] for x in critic_trace]),
                        quantiles=np.stack([x["quantiles"] for x in critic_trace]),
                        tau=tau.numpy(),
                    )
