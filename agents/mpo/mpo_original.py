    ...
    def _learn(self):
        ...
        with torch.no_grad():
            ...
            if self._use_retrace:
                ...
                # the value_est state need: s_t+1 to s_t+k
                # the improvement_state need: s_t to s_t+k-1  (the Q_w',t+k is not used in Q_t^ret calculation. bootstrapped)
                value_est_states = all_states[:, 1:, :] # s_t,.., s_k. only used to calculate retrace V
                improvement_states = s_seq # s_0,..s_k-1. used ot find Q_w',t+1 in td-error equation and Q_t^ret calculation
                
                # 1. sampling actions first time, and evaluating using Q to get V estimation
                flat_value_states = value_est_states.reshape(B * K, self.obs_dim)

                value_policy = self.actor_target(flat_value_states)
                value_actions = value_policy.sample((N,))  # [N, B*K, D]

                tiled_value_states = (flat_value_states.unsqueeze(0)
                    .expand(N, -1, -1)
                    .reshape(N * B * K, self.obs_dim))
                value_q_samples = self.target_critic(
                    tiled_value_states,
                    value_actions.reshape(N * B * K, D),
                ).reshape(N, B, K)
                v_t = value_q_samples.mean(dim=0)  # [B, K]

                # 2. using states and sampling 2nd set of actions for E/M steps
                # s_0,... s_k-1. these Q values = Q_n,b used in weight calculation
                policy_states = improvement_states.reshape(
                    B * K, self.obs_dim)
                target_policy = self.actor_target(policy_states)
                sampled_actions = target_policy.sample((N,))  # [N, B*K, D]

                tiled_policy_states = (
                    policy_states.unsqueeze(0)
                    .expand(N, -1, -1)
                    .reshape(N * B * K, self.obs_dim))
                q_values = self.target_critic(
                    tiled_policy_states,
                    sampled_actions.reshape(N * B * K, D),
                ).reshape(N, B * K)
...
def parse_args():
    ...
    parser.add_argument("--retrace_lambda",type=float,default=1.0,help="Retrace trace coefficient.")
