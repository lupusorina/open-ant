...
def make_eval_env(args: SimpleNamespace, xml_file: str | None = None):
    ...
    if is_gymnasium_env(args.env_id):
        ...
        if is_dm_control:
            # Match the environment construction used during training.
            gym_kwargs["task_kwargs"] = {
                "random": int(getattr(args, "seed", 0)),
            }
            gym_kwargs["environment_kwargs"] = {
                "flat_observation": True,
            }
        elif xml_file is not None:
            # Custom XML applies to regular Gymnasium MuJoCo environments,
            # not DeepMind Control Suite environments.
            gym_kwargs["xml_file"] = xml_file

        env = gym.make(args.env_id, **gym_kwargs)

        # Some dm_control/Shimmy versions still expose a Dict even when
        # flat_observation=True, so use the same fallback as training.
        if isinstance(env.observation_space, gym.spaces.Dict):
            env = gym.wrappers.FlattenObservation(env)

        if not isinstance(env.observation_space, gym.spaces.Box):
            raise TypeError(
                "Expected a Box observation space after flattening, but got "
                f"{env.observation_space!r}"
            )
