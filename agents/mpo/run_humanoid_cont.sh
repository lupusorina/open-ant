...
SCRIPT="mpo_acme.py"
GPU_ID="${GPU_ID:-2}"
OUTPUT_DIR="/data2/serenaliu_data/continual_mpo_humanoid_massfric"
MODEL_PATH="/home/serenaliu/caltech_linc_home/open-ant/sim/assets/humanoid_sim2.xml"
...
for seed in $(seq 0 0); do
    seed_dir="$(
        find "${RUNS_ROOT}" \
            -maxdepth 1 \
            -type d \
            -name "mpo_humanoid_*_seed_${seed}" |
            # -name "continual_mpo_humanoid_*_seed_${seed}" |
        sort |
        tail -n 1
    )"
