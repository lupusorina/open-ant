...
class PerWeightMetaAdam:
    ...
    def step(self):
        for p, state in zip(self.params, self.state):
            ...
            # use the post-updated alpha to update h and parameter p?
...
class NetworkMetaAdam:
    ...
    def __init__(
        ...
        for p in self.params:
            self.state.append({
                ...
                # Meta Adam state
            # the meta values should all be the same across all params of a network
        device = self.params[0].device
        dtype = self.params[0].dtype

        self.beta = torch.tensor(math.log(alpha0),dtype=dtype,device=device,)

        self.meta_m = torch.zeros((),
            dtype=dtype,device=device,)
        self.meta_v = torch.zeros(
            (),dtype=dtype,device=device)
        self.meta_step = 0
    ...
    def step(self):
        ...
        self.meta_step += 1
        self.meta_m.mul_(self.meta_b1).add_(z,alpha=1.0 - self.meta_b1)
        self.meta_v.mul_(self.meta_b2).add_(z * z,alpha=1.0 - self.meta_b2)

        meta_m_hat = (self.meta_m / (1.0 - self.meta_b1 ** self.meta_step))
        meta_v_hat = ( self.meta_v / (1.0 - self.meta_b2 ** self.meta_step))     
        
        self.beta.add_(-self.meta_lr * meta_m_hat / (torch.sqrt(meta_v_hat) + self.eps))
        self.beta.clamp_(self.min_beta, self.max_beta,)

        alpha = torch.exp(self.beta)
        self.step_num += 1
        ...
        for p, state in zip(self.params, self.state):
            ...
            v.mul_(self.b2).addcmul_(g, g, value=1.0 - self.b2)
            # bias corrections
            m_hat = m / (1.0 - self.b1 ** self.step_num)
            v_hat = v / (1.0 - self.b2 ** self.step_num)
    ...
    def state_dict(self):
        ...
        return {
            ...
            # Network-wide IDBD / Meta Adam
            "beta": self.beta.detach().cpu(),
            "meta_m": self.meta_m.detach().cpu(),
            "meta_v": self.meta_v.detach().cpu(),
...
class SAC:
    def __init__(self,
        ...
        self.q1_optimizer = NetworkMetaAdam(
        ...
        self.q2_optimizer = NetworkMetaAdam(
        ...
        self.actor_optimizer = NetworkMetaAdam(
        ...
        if self.autotune:
            ...
            # self.a_optimizer = PerWeightMetaAdam(
            #             [self.log_alpha],
            #             alpha0=alpha_lr,
            #             meta_lr=meta_lr,
            #             gamma=meta_gamma,
    ...
    def load_state(self, state_dict, load_extra_weights=False, reset_entropy_alpha=False):
        if not load_extra_weights:
            ...
            with torch.no_grad():
                ...
                self.actor.fc1.weight[:old_hidden].copy_(old_actor["fc1.weight"].to(self.device))
                self.actor.fc1.bias[:old_hidden].copy_(old_actor["fc1.bias"].to(self.device))
                ...
                # restore the old Sim1 block of weight/biases in matrix
                self.actor.fc2.weight[:old_hidden, :old_hidden].copy_(old_actor["fc2.weight"].to(self.device))

                self.actor.fc2.bias[:old_hidden].copy_(old_actor["fc2.bias"].to(self.device))

                # new 1st layer neurons shouldn't at first influence old 2nd layer neurons
                self.actor.fc2.weight[:old_hidden, old_hidden:].zero_()

                if self.actor.use_layer_norm:
                    # make new neurons [old_hidden:] layernorm parameters = PyTorch default,
                        # weight (gamma) = 1, bias   (beta)  = 0
        
                    self.actor.ln1.weight[:old_hidden].copy_(old_actor["ln1.weight"].to(self.device))
                    self.actor.ln1.bias[:old_hidden].copy_(old_actor["ln1.bias"].to(self.device))

                    # LayerNorm after fc2
                    self.actor.ln2.weight[:old_hidden].copy_(old_actor["ln2.weight"].to(self.device))
                    self.actor.ln2.bias[:old_hidden].copy_(old_actor["ln2.bias"].to(self.device))

                self.actor.fc_mean.weight[:, :old_hidden].copy_(old_actor["fc_mean.weight"].to(self.device))

                self.actor.fc_mean.bias.copy_(old_actor["fc_mean.bias"].to(self.device))

                self.actor.fc_mean.weight[:, old_hidden:].zero_()  # zero out new neurons' connections to original output

                self.actor.fc_logstd.weight[:, :old_hidden].copy_(old_actor["fc_logstd.weight"].to(self.device))

                self.actor.fc_logstd.bias.copy_(old_actor["fc_logstd.bias"].to(self.device))

                self.actor.fc_logstd.weight[:, old_hidden:].zero_()
                ...
                for critic, old_critic in critic_pairs:
                    ...
                    critic.fc1.weight[:old_hidden].copy_(old_critic["fc1.weight"].to(self.device))
                    ...
                    critic.fc2.weight[:old_hidden, :old_hidden].copy_(old_critic["fc2.weight"].to(self.device))
                    critic.fc2.bias[:old_hidden].copy_(old_critic["fc2.bias"].to(self.device))

                    # map from layer 1 new neurons to layer 2 old neurons = 0 initially.
                    critic.fc2.weight[:old_hidden, old_hidden:].zero_()

                    if critic.use_layer_norm:
                        critic.ln1.weight[:old_hidden].copy_(old_critic["ln1.weight"].to(self.device))
                        critic.ln1.bias[:old_hidden].copy_(old_critic["ln1.bias"].to(self.device))

                        # LayerNorm after fc2
                        critic.ln2.weight[:old_hidden].copy_(old_critic["ln2.weight"].to(self.device))
                        critic.ln2.bias[:old_hidden].copy_(old_critic["ln2.bias"].to(self.device))

                    critic.fc3.weight[:, :old_hidden].copy_(old_critic["fc3.weight"].to(self.device))

                    critic.fc3.bias.copy_(old_critic["fc3.bias"].to(self.device))

                    critic.fc3.weight[:, old_hidden:].zero_()
        ...
        if self.autotune: 
            if not reset_entropy_alpha and "log_alpha" in state_dict:
                ...
                print(
                    f"[√] Loaded entropy alpha from checkpoint: "
                    f"alpha={self.alpha:.6g}"
                )
            else:
                self.alpha = self.log_alpha.exp().item()
                print(
                    f"[√] Reset entropy alpha for new run: "
                    f"alpha={self.alpha:.6g}"
                )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="when loading a checkpoint, reset SAC entropy alpha and its optimizer instead of loading them",
    )
...
if __name__ == "__main__":
    ...
    if args.weights_path is not None:
        ...
        agent.load_state(state, load_extra_weights=args.load_extra_weights, reset_entropy_alpha=args.reset_entropy_alpha)
