...
RUNS_DIR = "/home/seliu/open-ant/agents/distributional-mpo/runs-ant/exp1"
...
for seed in SEEDS_TO_PLOT:
    sim1_dirs = sorted(glob.glob(f"{RUNS_DIR}/dmpo_retrace*_seed_{seed}"), reverse=True)
    sim2_dirs = sorted(glob.glob(f"{RUNS_DIR}/continuous_dmpo_retrace*_seed_{seed}"), reverse=True)
