...
def main():
   ...
   # set up the seeds
    ...
    beta_actor = [
        torch.full_like(
            p,
            float(np.log(args.actor_lr)),
            device=DEVICE,
        )
        for p in actor.parameters()
    ]

    # One beta_i per scalar critic parameter.
    beta_critic = [
        torch.full_like(
            p,
            float(np.log(args.critic_lr)),
            device=DEVICE,
        )
    ...
    # IDBD critic Update
            ...
            # save the gradients into a list. 
                ...
                with torch.no_grad():
                    critic_alphas = []
                    for p, g, h, beta in zip(critic.parameters(), critic_grads, h_critic, beta_critic):
                        beta += (args.critic_meta_lr * delta.detach() * (g * h))
                        beta.clamp_(min_beta, max_beta)
                        alpha = torch.exp(beta)

                        critic_alphas.append(alpha)

                        h += alpha * (delta.detach() - g*h) * g

                        # critic weight update: w += alpha(t+1) * delta * grad V
                        p += alpha * delta.detach() * g
        ...
        # IDBD actor Update
                ...
                with torch.no_grad():
                    ...
                    actor_alphas = []
                    for p, g, h, beta in zip(actor.parameters(), actor_grads, h_actor, beta_actor):
                        beta += (args.actor_meta_lr * delta.detach() * (g * h))
                        beta.clamp_(min_beta, max_beta)
                        alpha = torch.exp(beta)
                        actor_alphas.append(alpha)

                        h += alpha * (delta.detach()) * g   # no - g*h decay term for actor?
                                                        # h += alpha * (delta.detach() - g*h) * g 
                        # actor weight update: theta += alpha(t+1) * delta * grad log pi
                        p += alpha * delta.detach() * g
           ...
           # I = 1.0
                obs = next_obs_t
            mean_critic_lr = (
                sum(alpha.sum().item() for alpha in critic_alphas)
                /
                sum(alpha.numel() for alpha in critic_alphas)
            )

            critic_lr_sum += mean_critic_lr

            mean_actor_lr = (
                sum(alpha.sum().item() for alpha in actor_alphas)
                /
                sum(alpha.numel() for alpha in actor_alphas)
            )

            actor_lr_sum += mean_actor_lr
    ...
    torch.save({
        ...
        "beta_actor": [
            beta.detach().cpu()
            for beta in beta_actor
        ],

        "beta_critic": [
            beta.detach().cpu()
            for beta in beta_critic
        ],

        "actor_lr_final": [
            torch.exp(beta).detach().cpu()
            for beta in beta_actor
        ],
