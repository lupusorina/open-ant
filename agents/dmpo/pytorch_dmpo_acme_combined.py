...
class MPO:
    def __init__(self, args, envs, disk_folder='', run_name=None, runs_directory='runs'):
        ...
        for d in (self.run_dir, self.log_dir, self.weights_folder, self.video_dir):
        ...
        self.keys_agent_vars = [
            ...
            'utd', 'SPS', 'average_reward_per_second', 'reward',
        ]
    ...
    def agent_step(self, next_obs, actions, rewards, terminations, truncations, infos):
        ...
        valid = ~torch.as_tensor(infos["autoreset"], dtype=torch.bool, device=self.device)
        self.rb.add(
            self.obs,
            next_obs,
            actions,
...
def parse_args():
    ...
    parser.add_argument("--eval", action="store_true", help='evaluating checkpoint')
    parser.add_argument("--weights_path", type=str, default=None)
    parser.add_argument("--seed", type=int, default=0, help="fix seed for run")
    parser.add_argument("--cuda", action="store_true", help="use gpu")
    parser.add_argument("--torch_deterministic", type=bool, default=False)
    parser.add_argument("--total_timesteps", type=int, default=1_000_000_000)
    parser.add_argument("--num_envs", type=int, default=2048)
    parser.add_argument("--env_name", type=str, choices=["elec","hydro"], default="hydro", help="env id")
    parser.add_argument("--buffer_size", type=int, default=int(5e6))
    ...
    parser.add_argument("--runs_directory", type=str, default="runs",
                        help="root directory under which each run dir is created")
...
def make_env(args, exp_name):
    if args.env_name == "elec":
        env_raw = crane.VectorEnv(crane.CraneSim(), num_envs=args.num_envs)
    elif args.env_name == "hydro":
        env_raw = crane.VectorEnv(crane.CraneKnuckleBoomSim(), num_envs=args.num_envs)
        # env_raw = crane.SaveVideoWrapper(env_raw, f"videos/crane_knuckle_boom_{exp_name}")
    else:
        env_raw = gym.make_vec(args.env_name, num_envs=args.num_envs,
...
def main():
    ...
    if args.train:
        experiment_name = datetime.now().strftime("%y-%m-%d_%H-%M-%S-%f") + f"_seed{seed}" + "_" + args.train.replace(" ", "_")
    else:
        experiment_name = "eval"

    print(f"\033]2;{experiment_name}\033\\", end="", flush=True)

    raw_env, env = make_env(args, experiment_name)
    ...
    # experiment_dir = os.path.join(args.runs_directory, experiment_name)
    # os.makedirs(experiment_dir, exist_ok=True)
    # with open(os.path.join(experiment_dir, "args.json"), "w") as f:
    #     json.dump(vars(args), f, indent=2)
 
    agent = MPO(args=args, envs=env, disk_folder='', run_name=experiment_name, runs_directory=args.runs_directory)
    ...
    obs, infos = raw_env.reset(seed=seed) 
    train_steps = int(np.round(args.total_timesteps/args.num_envs))
    for step in tqdm(range(agent.global_step, train_steps)):

        selected_actions = agent.get_action(obs, args.eval)
        next_obs, rewards, terminations, truncations, infos = env.step(selected_actions)
        ...
        if writer is not None and step % args.log_interval == 0:
            writer.add_scalar("reward/mean", float(rewards.mean()), step)
            if "benchmark_reward" in infos:
                writer.add_scalar("reward/benchmark",
                                    float(np.asarray(infos["benchmark_reward"]).mean()), step)
            writer.add_scalar("perf/buffer_size", agent.rb.size(), step)
            if metrics is not None:
                for k, v in metrics.items():
                    if isinstance(v, (int, float)):
                        writer.add_scalar(f"agent/{k}", v, step)
