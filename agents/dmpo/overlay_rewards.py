...
RUNS_DIR = "/home/serenaliu/caltech_linc_home/open-ant/agents/dmpo/runs/spi1536_gamma92"
...
for seed in SEEDS_TO_PLOT:
    sim1_dirs = sorted(glob.glob(f"{RUNS_DIR}/dmpo_*_seed_{seed}"), reverse=True)
    sim2_dirs = sorted(glob.glob(f"{RUNS_DIR}/continuous_dmpo_*_seed_{seed}"), reverse=True)
